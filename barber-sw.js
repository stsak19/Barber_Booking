...
async function networkFirst(request) {
  try {
    ...
    if (hit) return hit;
    const shell = await caches.match("./barber.html");
...
/* Το όνομα του καταστήματος. Ο service worker δεν βλέπει τις ρυθμίσεις,
...
self.addEventListener("push", (event) => {
  event.waitUntil((async () => {
    ...
    await self.registration.showNotification(payload.title || brand, {
      ...
      icon: "./icon-192.png",
      badge: "./icon-192.png",
      tag: payload.tag || "barber",
      data: { url: payload.url || "./barber.html" },
    });
  })());
});
...
const samePage = (a, b) => {
  const tidy = (p) => p.replace(/\/(barber\.html)?$/, "/");
  return tidy(a) === tidy(b);
};

self.addEventListener("notificationclick", (event) => {
  event.notification.close();
  const target = (event.notification.data && event.notification.data.url) || "./barber.html";
